...
\anote{TODO: make sure links to sites etc are not part of biblio -- only use biblio for academic references.  put urls inline \url{blah} or as footnotes}

\anote{TODO: redo figures for legibility and align thenm with text}
...
We then use our own prototype implementation of the  marketplace model on a private Ethereum network, to experimental evaluate the cost/risk trade-offs that are available by setting checkpoint frequency, also taking advantage of \textit{potential} external mechanisms for establishing trust amongst participants, \textit{if they are} available.


requirements. It is a challenging task to have a public decentralized marketplace dealing with a live stream with no trusted component.
...
\mynote{make shorter and more concise}
...
\mynote{integrate this in sec. II}
