...
When both parties comply with the agreement and data transfer takes place as expected, at the end of the \tati{} interval $C$ informs the smart contract \sc{} that the agreement has been fulfilled, and \sc{} proceeds to settle the payment as per the agreement.
Suppose however that $C$ fails to inform \sc. This may be because $C$ actually failed to receive some of the data in the stream, or because it fraudolently \textit{claims} not to have received the data.
In our marketplace, we assume that \sc{} is unable to distinguish between these two events, i.e., there is no requirement for the data broker to keep a (verifiably truthful) log of its message delivery.
In this situation, the only possible course for \sc{} is to believe $C$'s claim, and to withhold $P$'s payment as a consequence.
...
According to this protocol, at each checkpoint $C$ is expected to send a \textit{data receipt} to \sc as part of a blockchain transaction, which acknowledges receipt of one batch of data from $P$.
When this happens, sc{} records the receipt and then informs $P$. 
if $P$ does not receive a message from \sc, it eventually times out and stops the stream (in practice, $C$'s subscription to the stream is cancelled), effectively terminating the trade agreement.

This mechanism provides an upper bound to the loss of data incurred by $P$ in case of fraud, which is a function of \bs, as that determines the distance between two checkpoints (by ``loss of data'' here we mean, $P$ providing streaming messages to $CR$ without getting paid for them).
Importantly, when using blockchains that are incur a fee for executing smart contracts, like Ethereum, there is a trade-off between the risk of losing data and the cost of engaging in a long-running transaction with many checkpoints along the way, because each checkpoint incurs a transaction fee.
In our experimental evaluation (Sec.~\ref{sec:evaluation}) we explore this trade-off in detail. 
in the rest of this section, however, we provide a theoretical analysis of this protocol.
