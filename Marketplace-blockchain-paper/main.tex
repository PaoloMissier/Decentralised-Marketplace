...
Suppose however that $C$ fails to inform \smartc. This may be because $C$ actually failed to receive some of the data in the stream, or because it fraudolently \textit{claims} not to have received the data.
...
By setting \bs{} to a small value, data loss will be limited to one or at most a few data batches (see below for details). 
However, a large \bs{} exposes $P$ to a higher risk of fraud.
Intuitively, $P$ will set \bs{} to a value that is determined by how much $C$ can be trusted. 
In the following we make this intuition precise by establishing a connection between \bs{} and the participants' reputation score.

Setting \bs{} correctly for each transaction is critical to achieve a sustainable marketplace, because checkpoints can be costly as they are smart contract transactions, which in blockhain models like Ethereum incur a fee. There is therefore a trade-off between the risk of losing data and the cost of engaging in a long-running transaction with many checkpoints along the way.
In our experimental evaluation (Sec.~\ref{sec:evaluation}) we explore this trade-off in detail. 
In the rest of this section, however, we provide a theoretical analysis of this simple protocol.
